...
def school_grading(name:str, scores:list):
    sum=0
    for i in scores:
        sum+=i
    if sum/len(scores)>=60:
        print(name,"--> avarage:",sum/len(scores),"\nCongratulations you passed the exam.\n")
    else:
        print(name,"--> avarage:",sum/len(scores),"\nI'm sorry you failed the exam.\n")

classA=[["Pino",[67,22,44,100,26,78]],["Gino",[92,74,51,27,96,56]],["Caio",[44,75,24,97,13,48]],
        ["Tizio",[34,23,12,56,80,66]],["Sempronio",[90,89,67,88,99,45]],["Lillo",[73,60,44,32,78,29]]]
for i in classA:
    school_grading(i[0],i[1])
...
def guess_number(ranges, attempts):
    num=randrange(ranges[0],ranges[1])
    for i in range(attempts):
        guess=int(input("Insert your guess for the number: "))
        if guess==num:
            print("Congratulationsss, you got it right!!!")
            return
        elif guess<num:
            print("Wrong, the number is higher.")
        else:
            print("Wrong, the number is lower.")
    print("It seems you failed to guess the right number, it was:",num)
    print("Don't worry you can always try again.")

ranges=[]
print("Please insert the range in wich we'll generate a random number.")
ranges.append(int(input("Lowest number: ")))
ranges.append(int(input("Highest number: ")))
attempts=int(input("Insert the number of tries you'll have to guess the correct number."))
guess_number(ranges, attempts)
...
14. Sudoku Solver:
    ...
    Implement a backtracking algorithm to check for valid placements in empty cells, ensuring no row, column, 
    or 3x3 subgrid contains duplicates.
